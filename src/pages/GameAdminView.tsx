...
import { ArrowRight, Calendar, Check, CheckCircle, DollarSign, Share2, Users, X } from "lucide-react";
    ...
    <>
      ...
      <div className="pb-24 animate-fade-in-up px-4 md:px-6 lg:px-24">
        ...
        <div className="flex items-center gap-3 mb-4">
          <button onClick={handleBack} className="p-2 bg-white/10 rounded-lg hover:bg-white/20 transition-colors"><ArrowRight className="w-5 h-5 rotate-180" /></button>
          ...
          {MOCK_PARTICIPANTS.map(p => (
            <div key={p.id} className="p-4 flex items-center justify-between hover:bg-slate-50 transition-colors">
              ...
              <div className="flex flex-col items-end gap-2">
                  {p.payment === 'pending' ? (
                     <button className="text-xs flex items-center gap-1 bg-white text-slate-600 font-bold px-3 py-1.5 rounded-lg border border-slate-200 hover:border-emerald-500 hover:text-emerald-600 hover:bg-emerald-50 transition-all shadow-sm group">
                         <div className="bg-slate-100 rounded-full p-0.5 group-hover:bg-emerald-100 transition-colors">
                  ...
                  {p.status === 'pending' ? (
                      <div className="flex gap-2">
                        <button
                          onClick={() => setConfirmModal({ isOpen: true, type: 'reject', participantName: p.name })}
                        ...
                        <button
                          onClick={() => setConfirmModal({ isOpen: true, type: 'approve', participantName: p.name })}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in-up">
          <div className="bg-white rounded-2xl p-6 shadow-2xl max-w-sm w-full">
            <h3 className="text-lg font-bold text-slate-900 mb-2">
              {confirmModal.type === 'approve' ? 'Aprovar Visitante' : 'Reprovar Visitante'}
            </h3>
            <p className="text-slate-600 mb-6">
              Tem certeza que deseja {confirmModal.type === 'approve' ? 'aprovar' : 'reprovar'} a entrada de <strong>{confirmModal.participantName}</strong>?
            </p>
            <div className="flex gap-3">
              <Button variant="ghost" className="flex-1" onClick={() => setConfirmModal({ ...confirmModal, isOpen: false })}>
                Cancelar
              </Button>
              <Button 

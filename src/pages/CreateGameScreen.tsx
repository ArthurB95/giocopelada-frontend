...
const CreateGameScreen: React.FC<CreateGameScreenProps> = ({ onCancel }) => {
  const [allowVisitors, setAllowVisitors] = useState(false);
      ...
      <div className="max-w-2xl mx-auto pb-24 animate-fade-in-up">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
            ...
            <div className="flex items-center justify-between py-2">
                ...
                <div 
                    onClick={() => setAllowVisitors(!allowVisitors)}
                    className={`w-11 h-6 rounded-full relative cursor-pointer transition-colors ${allowVisitors ? 'bg-emerald-500' : 'bg-slate-200'}`}
                >
                    <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${allowVisitors ? 'right-1' : 'left-1'}`}></div>
                </div>
            </div>

            {allowVisitors && (
                <div className="mt-4 p-4 bg-slate-50 rounded-xl border border-slate-200 animate-fade-in-up">
                    <p className="text-sm font-medium text-slate-700 mb-3">Liberar lista de espera:</p>
                    <div className="flex items-center gap-3">
                        <input 
                            type="number" 
                            className="w-20 px-3 py-2 rounded-lg border border-slate-200 focus:border-emerald-500 outline-none text-center"
                            placeholder="24"
                        />
                        <select className="flex-1 px-3 py-2 rounded-lg border border-slate-200 focus:border-emerald-500 outline-none bg-white">
                            <option value="hours">horas</option>
                            <option value="days">dias</option>
                        </select>
                        <span className="text-sm text-slate-500 whitespace-nowrap">antes do jogo</span>
                    </div>
                </div>
            )}
